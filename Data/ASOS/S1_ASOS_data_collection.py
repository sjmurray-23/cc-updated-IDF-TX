...
"""
Code extracted originally from https://arm-doe.github.io/ACT/_modules/act/discovery/get_asos.html
    dead end link so see https://github.com/ARM-DOE/ACT/blob/v1.5.0/act/discovery/get_asos.py
to use latest option see https://arm-doe.github.io/ACT/_modules/act/discovery/asos.html
OR locate at https://github.com/ARM-DOE/ACT
...
def get_asos(time_window, lat_range=None, lon_range=None, station=None):
    ...
    for stations in site_list:
        ...
        if len(my_df['lat'].values) == 0:
            ...
            my_df['p01i'] = my_df['p01i']
            my_df['p01i'].attrs['units'] = 'inch'
            my_df['p01i'].attrs['long_name'] = '1 hour precipitation'
...
address_csvfilename= 'Data/ASOS/'
# file with TX ASOS stations that are on land and have data up to the end of 2022; prepared previously
Station_CSVFilenames="TX_ASOS_Stations_CleanedClipped.csv"
TX_Stations_PD=pd.read_csv(address_csvfilename+Station_CSVFilenames, index_col=False) 
TX_Station_namesList= list(TX_Stations_PD.iloc[:, 0])
##### adding only funcational gages to the list
TX_Station_list_functional=[]
for st in TX_Station_namesList:
    if st not in ['NGW','18H','ORC']:
        TX_Station_list_functional.append(st)
Station_Time_Dic={}
for station in TX_Station_list_functional:
    print(station)
    Station_Time_Dic[station]=[TX_Stations_PD['begin_month'].loc[TX_Stations_PD['stid']==station].item(),\
                      TX_Stations_PD['begin_day'].loc[TX_Stations_PD['stid']==station].item(),\
                          TX_Stations_PD['begin_year'].loc[TX_Stations_PD['stid']==station].item()]

# create saving directory if it does not already exist
saving_dir = address_csvfilename+'Raw_Precipitation_Data/'
# Check if the directory exists before creating it
if not os.path.exists(saving_dir):
    os.makedirs(saving_dir)

# sorted_Station_Time_Dic_2 = sorted(Station_Time_Dic.items(), key=lambda x:x[1][2])        
for st in Station_Time_Dic:
    StartTime = datetime(int(Station_Time_Dic[st][2]), int(Station_Time_Dic[st][0]), int(Station_Time_Dic[st][1]),0)
    EndTime=datetime(2022,12,31,0)
    time_window=(StartTime,EndTime)
    asos_ds= get_asos(time_window, lat_range=None, lon_range=None, station=st)
    asos_df=asos_ds[st]['p01i'].to_dataframe()
    asos_df = asos_df.rename(columns={'p01i': st})
    asos_df.to_csv(saving_dir+st+'_'+str(Station_Time_Dic[st][2])+'_Precipitation.csv')
